...
export const ServerConfigModal: React.FC<ServerConfigModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-app-bg/80 backdrop-blur-sm animate-fadeIn">
      <div className="bg-app-surface border border-app-border rounded-2xl w-full max-w-lg p-6 shadow-2xl shadow-black/80 relative max-h-[90vh] overflow-y-auto">
        <button
          onClick={onClose}
        ...
        {savedSuccess && (
          <div className="mb-4 p-3 rounded-xl bg-app-bg/60 border border-app-status-nominal/30 text-emerald-300 text-xs flex items-center gap-2">
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              />
              <Server className="w-4 h-4 text-app-text-primary0 absolute left-3.5 top-1/2 -translate-y-1/2" />
            </div>
            <span className="text-[11px] text-app-text-primary0 mt-1 block">
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-app-bg border border-app-border-highlight rounded-xl px-3.5 py-2.5 pl-10 text-app-text-primary placeholder-app-text-muted focus:outline-none focus:border-app-accent font-mono text-xs"
              />
              <Key className="w-4 h-4 text-app-text-primary0 absolute left-3.5 top-1/2 -translate-y-1/2" />
            </div>
          ...
          <div className="p-3.5 bg-app-bg/80 border border-app-border rounded-xl space-y-2.5">
            <div className="flex items-center justify-between">
              ...
              {normalizedActive ? (
                isTb ? (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-app-bg/80 text-emerald-300 border border-app-status-nominal/30">
                    <ShieldCheck className="w-3.5 h-3.5 text-app-status-nominal" />
                    ThingsBoard JWT Active
                  </span>
                ) : isOidc ? (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-app-bg/80 text-app-accent border border-app-accent/30">
                    <ShieldAlert className="w-3.5 h-3.5 text-app-accent" />
                    Authentik OIDC (Causes 401 in TB)
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-app-bg/80 text-app-status-info border border-app-status-info/30">
                    Custom JWT
                  </span>
                )
              ) : (
                <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-app-bg/80 text-rose-300 border border-app-status-critical/30">
            ...
            <div className="pt-2 flex items-center justify-between border-t border-app-surface">
              ...
              {onOpenDiagnostics && (
                ...
                >
                  <Terminal className="w-3.5 h-3.5 text-app-text-primary0" />
            ...
            {testResult && (
              <div
                className={`p-2.5 rounded-lg text-xs font-mono border ${
                  testResult.success
                    ? 'bg-app-bg/60 border-emerald-800/60 text-emerald-300'
                    : 'bg-app-bg/60 border-rose-800/60 text-rose-300'
                }`}
              >
                {testResult.success
            ...
            {currentUser && (
              <div className="pt-1 text-[11px] text-app-text-primary0 font-mono flex items-center justify-between border-t border-app-surface">
          ...
          <div className="pt-3 border-t border-app-border flex items-center justify-end gap-3">
            ...
            <button
              type="submit"
              className="px-5 py-2 rounded-xl text-xs font-bold bg-app-accent-hover hover:bg-app-accent text-app-accent-text transition-all shadow-md shadow-app-bg/40 cursor-pointer"
            >
              Save Settings
            </button>
          </div>
