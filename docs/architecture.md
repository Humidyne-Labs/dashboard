# HUMID1 Frontend Dashboard — Architecture & Design Specification

Welcome to the frontend system architecture and engineering specification for **HUMID1**, designed and engineered by **Humidyne Labs**. This document outlines the structure, state management, security boundaries, and rendering engines powering the HUMID1 client dashboard.

---

## 1. System Topology & Decoupled Frontend

The HUMID1 Dashboard is a production-grade, highly-optimized React Single Page Application (SPA). Rather than directly interfacing with an application database or managing hardware nodes, the dashboard operates as a decoupled, stateless client that interfaces with ThingsBoard CE (IoT Core) and Authentik (Identity Provider) over standard HTTPS and REST:
...
HUMID1's visual architecture is focused on readability, high-contrast diagnostics, and an aesthetic suited for premium humidor hardware (slate tones with amber/gold accents):

- **Color Foundations:** Uses rich dark tones (`#020617` to `#0f172a`) with subtle warm gray dividers (`#1e293b`) to represent cedarwood structures, combined with amber alert accents (`#f59e0b`).
- **Bento Grid Architecture:** Responsive layout that scales from small mobile screens to large desktop monitors. Each widget is self-contained with its own internal state:
  ...
  7. **Alarms Management Feed (`AlarmsFeed.tsx`):** Reports active and historic hardware warnings, integrating visual trigger indicators and manual operators to acknowledge (`POST /api/alarm/{id}/ack`) or clear (`POST /api/alarm/{id}/clear`) alerts.
...
- **Canonical Kelvin Threshold Engine (`alarmThresholds.ts`):** Temperature alarm thresholds are stored canonically in Kelvin (K) in both local runtime state and ThingsBoard shared attributes (`alarm_thresholds`). User-facing display is strictly presented in °F or °C (`toDisplayTemp` / `fromDisplayTemp`), preserving threshold integrity and enabling unit-agnostic ThingsBoard rule chain evaluation without rounding drift.
- **LTTB Downsampling Engine (`downsample.ts`):** High-density time-series history arrays are downsampled client-side using the Largest-Triangle-Three-Buckets (LTTB) algorithm before passing data to Recharts, ensuring smooth rendering even over multi-day spans.
- **Web Push & Audio Synthesizer (`pushNotifications.ts`, `notificationService.ts`):** In-app synthesized audio alerts and Web Push API service worker notifications alert users when humidity or temperature breaches warning/critical envelopes.
...
- **Flexible Device Discovery:** Detects the active user profile authority. If a `CUSTOMER_USER`, it queries sandboxed customer directories; if a `TENANT_ADMIN`, it queries global tenant-level indices, preventing permission/SSO mismatch exceptions.
...
1. **Authentik OIDC Protocol:** Users complete authentication on the Authentik single-sign-on portal using Authorization Code Flow with PKCE.
...
The frontend code compiles into a standard static distribution or packages into mobile app wrappers:

- **Progressive Web App (PWA):** Implements standard web manifests, offline app shell caching via `vite-plugin-pwa` (Workbox), and responsive viewport guidelines.
