...
jobs:
  build-android-twa:
    ...
    steps:
      ...
      # Non-interactive configuration for Bubblewrap & Android SDK compatibility
      - name: Configure Bubblewrap Paths & SDK Compatibility
        run: |
          mkdir -p ~/.bubblewrap
          echo "{\"jdkPath\":\"$JAVA_HOME\",\"androidSdkPath\":\"$ANDROID_HOME\"}" > ~/.bubblewrap/config.json
          
          # Bubblewrap validates $ANDROID_HOME by checking for a 'tools' or 'bin' folder.
          # In modern Android SDK images, tools are located under cmdline-tools/{version,latest}.
          CMDLINE_DIR="$ANDROID_HOME/cmdline-tools/latest"
          if [ ! -d "$CMDLINE_DIR" ]; then
            CMDLINE_DIR=$(ls -d $ANDROID_HOME/cmdline-tools/* 2>/dev/null | sort -V | tail -n 1)
          fi
          
          if [ -n "$CMDLINE_DIR" ] && [ -d "$CMDLINE_DIR" ]; then
            echo "Linking cmdline-tools ($CMDLINE_DIR) to $ANDROID_HOME/tools and $ANDROID_HOME/bin for Bubblewrap compatibility..."
            sudo ln -sf "$CMDLINE_DIR" "$ANDROID_HOME/tools"
            sudo ln -sf "$CMDLINE_DIR/bin" "$ANDROID_HOME/bin"
          fi
          
          # Ensure runner user has write permissions in ANDROID_HOME for build-tools
          sudo chown -R runner:docker "$ANDROID_HOME"
      ...
      - name: Build Android TWA Project with Bubblewrap
        env:
          BUBBLEWRAP_KEYSTORE_PATH: "${{ github.workspace }}/android-build/android-keystore.jks"
        ...
        run: |
          ...
          # Generate native files non-interactively preserving twa-manifest.json versioning
